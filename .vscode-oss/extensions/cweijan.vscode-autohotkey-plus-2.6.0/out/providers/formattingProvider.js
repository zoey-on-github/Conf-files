...
class FormatProvider {
    provideDocumentFormattingEdits(document, options, token) {
        ...
        for (let line = 0; line < document.lineCount; line++) {
            ...
            ;
            const purityText = codeUtil_1.CodeUtil.purity(originText.toLowerCase());
            let notDeep = true;
            ...
            if (notDeep) {
                for (const oneCommand of FormatProvider.oneCommandList) {
                    let temp;
                    if ((temp = new RegExp("\\b" + oneCommand + "\\b(.*)").exec(purityText)) != null
                        && !temp[1].includes("/")) {
                        oneCommandCode = true;
                        deep++;
                        break;
...
FormatProvider.oneCommandList = ["ifnotexist", "ifexist", "ifwinactive", "ifwinnotactive",
